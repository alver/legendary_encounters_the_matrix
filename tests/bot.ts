...
export const botUI: UIPort = {
  ...
  async confirmBox() { return Math.random() < 0.7; },
};
...
async function botActionPhase(cheat: boolean) {
  ...
  while (g().phase === 'action' && !g().gameOver && guard++ < 80) {
    ...
    if (P().hand.length) { await actPlayCard(P().hand[0].uid); continue; }
    // 2. resolve pendings
    if (g().pending.length) { await actPending(0); continue; }
    // 3. finale: raise the time track
    if (g().act === 3 && g().part === 3 && g().time < 10 && P().A >= g().time + 1) { await actRaiseTime(); continue; }
    ...
    if (g().act === 1 && g().part === 2 && P().rsi === 'real'
      ...
      await actFreeNeo(); continue;
    }
    if (g().attached.building && inMatrix() && !g().matrixRow[3]) { await actJump(); continue; }
    // 6. challenges
    if (inMatrix()) {
      const inPlayCards = [...g().operations, ...g().combatZone, ...g().matrixRow.filter(x => x !== null)];
      ...
      // Real World: recruit
      if (g().hovercraftStack.length && P().R >= 3 && Math.random() < 0.6) { await actRecruitHovercraft(); continue; }
      for (let i = 0; i < 5; i++) {
        const c = g().dock[i];
        if (c && P().R >= (D(c).cost ?? 0)) { await actRecruitDock(i); did = true; break; }
      }
      if (did) continue;
    }
